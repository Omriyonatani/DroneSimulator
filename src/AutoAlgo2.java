	...
	*/
    ...
    public void ai(int deltaTime) {
        ...
        if (SimulationWindow.return_home) {
            ...
            int closestIndex = getClosestIndex(points, drone.getOpticalSensorLocation());
            Point closest = new Point(points.get(closestIndex));
            while (points.size() > closestIndex+1) {
                removeLastPoint();
            }
            double orientationNeeded = Tools.getRotationBetweenPoints(drone.getOpticalSensorLocation(), closest);
            double toSpin = (drone.getGyroRotation() - orientationNeeded);
            if (toSpin > 5 || toSpin < -5) {
                speedDown();
                spinBy(toSpin,true);
            }else {
                speedUp();
            ...
            return;
            /*
            if (Tools.getDistanceBetweenPoints(drone.getOpticalSensorLocation(), closest) < max_distance_between_points) {
                System.out.println("orientations needed: " + orientationNeeded);System.out.println("to spin: " + toSpin);
                if (isRotating==0 && (toSpin > 2 || toSpin < -2)) {
                    System.out.println("spinning by: " + toSpin);
                    spinBy(toSpin,true);
                    try {
    ...
    public int getClosestIndex(ArrayList<Point> pointsList, Point p) {
        int closest = pointsList.size()-1;
        for (int i=pointsList.size()-2; i>=0;i--) {
            double dist = Tools.getDistanceBetweenPoints(p,pointsList.get(i));
            if (dist < Tools.getDistanceBetweenPoints(p, pointsList.get(closest))) {
                closest = i;
            }
        }
        return closest;
